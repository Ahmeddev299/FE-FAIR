// src/app/(leases)/ClauseManagement.tsx
'use client';
...
import  LeaseDetailsModal  from '@/components/models/LeaseDetailsModel'
...
// 👈 make sure this thunk hits /clause/get_user_leases?page=&limit=
import { getallUserLeasesAsync } from '@/services/lease/asyncThunk';

// 👈 from the clause async thunks we set up earlier
...
export default function ClauseManagement() {
    ...
    // redux state
    const { leaseList, isLoading, leaseError } = useSelector(selectLease);
    ...
    const [selectedLeaseDetails, setSelectedLeaseDetails] = useState<Lease | null>(null)

    // initial fetch
    useEffect(() => {
        dispatch(getallUserLeasesAsync({ page, limit }) as any);
    }, [dispatch, page, limit]);
    ...
    const totalCount =
        selectedLease?.totalCount ??
        (selectedLease?.clauses.length || 0);

    const unresolvedCount =
        selectedLease?.unresolvedCount ??
        (selectedLease?.clauses.filter((c) => c.unresolved).length || 0);

    // actions
    const onEdit = (clause: Clause) => {
        setSelectedClause(clause);
        setShowClauseDetail(true);
    };

    // optimistic helper (local update of selectedLease only)
    const optimisticallySetClauseStatus = useCallback((clauseId: Clause['id'], status: ClauseStatus) => {
    ...
    if (leaseError) return <div className="p-6 text-red-600">{leaseError}</div>;
        ...
        <DashboardLayout>
            ...
            {showDocumentPreview && (
                <DocumentPreviewModal
                    ...
                    pending={(selectedLease?.clauses ?? []).filter((c) => c.status !== 'Approved' && c.status !== 'Review')}
                    rejected={(selectedLease?.clauses ?? []).filter((c) => c.status === 'Review')} // adjust if you add explicit 'Rejected'
                />
            )}
            ...
            {showLeaseDetails && selectedLeaseDetails && (
                <LeaseDetailsModal
                    lease={selectedLeaseDetails}
